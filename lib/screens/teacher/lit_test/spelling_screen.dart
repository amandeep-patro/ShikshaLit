...
class _SpellingScreenState extends State<SpellingScreen> {
  ...
  void _removeLetter(int index) {
    ...
    if (index >= 0 &&
        ...
        selectedLetters[index].isNotEmpty) {
      setState(() {
  ...
  Future<void> _finishTest() async {
    ...
    await ScoresService.saveTestScore('spelling', score);
  ...
  Widget build(BuildContext context) {
    ...
    double letterBoxSize = min(
      ...
      max(
        30.0, // Minimum size
        (availableWidth - (currentWord.length - 1) * spacing) /
            currentWord.length,
      ),
    );
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color:
              isShowingResult
                  ? (selectedLetters[index].toLowerCase() ==
